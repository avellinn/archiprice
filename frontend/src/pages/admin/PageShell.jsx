  ...
  return (
    <section className="admin-stat-grid" aria-label="Résumé">
      {items.map((item, index) => (
        <article key={`${item.label}-${index}`} className={`admin-stat-card admin-stat-card--${item.tone || 'blue'}`}>
  ...
  return (
    <aside className="admin-filter-panel">
      ...
      {groups.map((group, groupIndex) => (
        <label key={`${group.label}-${groupIndex}`} className="admin-filter-field">
          <span>{group.label}</span>
          <select defaultValue={group.options[0]}>
            {group.options.map((option, optionIndex) => (
              <option key={`${group.label}-${option}-${optionIndex}`}>{option}</option>
            ))}
          </select>
        </label>
      ))}
